...
class Boosting:
    ...
    def train(self):
        ...
        for i in range(self.num_iterations):
            ...
            error = sum(
                map(lambda img_idx: weights[img_idx] if self.ytrain[img_idx] != h.predict(self.Xtrain[img_idx]) else 0,
                    range(len(self.ytrain))))
            # predicated = h.predict(self.Xtrain)
            # # find cumulative weights where prediction not equal to train
            # idx = np.subtract(predicated, self.ytrain)!=0.0
            # error = np.sum(weights[idx])
            ...
            if error > self.eps:
                ...
                weights = np.array(list(map(lambda img_idx: weights[img_idx] * np.exp(
                    (-1 * self.ytrain[img_idx] * alpha * h.predict(self.Xtrain[img_idx]))), range(len(weights)))))
...
class ViolaJones:
    ...
    def train(self, num_classifiers):
        ...
        for i in range(num_classifiers):
            ...
            weights = np.array(list(map(
                lambda img_idx: weights[img_idx] * beta if self.labels[img_idx] != h.predict(scores[img_idx]) else
                weights[img_idx] * 1.0, range(len(self.integralImages)))))
            # calculate the alpha
    ...
    def predict(self, images):
        ...
        for x in scores:
            ...
            left = sum(
                map(lambda idx: self.alphas[idx] * self.classifiers[idx].predict(x), range(len(self.classifiers))))
