...
def houghcircles(img, threshold, minR, maxR):
    circles = cv2.HoughCircles(img, cv2.cv.CV_HOUGH_GRADIENT, 1, threshold, param1=20, param2=10, minRadius=minR,
                               maxRadius=maxR)
    circle_coordinates = []
                    ...
                    if (line3 != line2 and line3 != line1) and abs(abs(line2[1] - line3[1]) - np.pi / 2.0) < threshold \
                        ...
                        for line4 in coordinates:
                            ...
                            if (line4 != line3 and line4 != line2 and line4 != line1) and abs(
                                    abs(line3[1] - line4[1]) - np.pi / 2.0) < threshold \
...
def traffic_sign_detection_challenge(img_in):
    ...
    for sign in signs:
        ...
        if sign == 'traffic_light':
            ...
            result = img_in.copy()
            circles = cv2.HoughCircles(edges, cv2.cv.CV_HOUGH_GRADIENT, 1, 2000, param1=80, param2=30, minRadius=50,
                                       maxRadius=130)
            coordinates = []
            if circles is not None:
                circles = circles.astype(int)
