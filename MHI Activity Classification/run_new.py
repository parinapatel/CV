    ...
    for i, action in enumerate(actions):
        ...
        for j,file in enumerate(files_path):
            if ".avi" in file:
                ...
                for iter in range(3):
                    binaries, real = util.create_binary_images(os.path.join("./Data MHI/"+str(act)+"/",file), k=(5,) * 2, sigma=0,
                                                               start_frame=start[iter], end_frame=end[iter],
                                                               k_size=(3,) * 2, threshold=Th[i], folder=act + "_" + str(j))
...
# 1. build dataset
fields = ["labels","MHI","MEI", "mus_mhi","vs_mhi","mus_mei","vs_mei"]
# data = build_entire_dataset(actions)
# if debug:
#     print("1. Build Dataset - Done")
#
# # 2. Save it as numpy
# if not os.path.exists("Data NPY Different T"):
#     os.makedirs("Data NPY Different T")
# for i, filename in enumerate(fields):
#     np.save(os.path.join("./Data NPY Different T/"+filename+".npy"), data[i])
# if debug:
...
data_loaded = []
for field in fields:
    np_data = np.load(os.path.join("Data NPY Different T",field+".npy"))
...
cv_scores = cross_val_score(knn_cv, data_loaded[3],data_loaded[0], cv=100)
...
mu, v = util.get_features(test_video_path, start_frame, end_frame, Th, T)
if debug:
    print("6. create features from test video - Done")
    print(v)

# 7. test model
predictions = knn_cv.predict(v)
if debug:
